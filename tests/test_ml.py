...
from app.ml.prediction import (
    ...
    _all_genres_list, _all_personnel_list, _all_studios_list, # Access loaded ref data
    _model # Access mock model
)
from app.static_data_loader import get_historical_films, get_genres, get_cast_and_crew, get_studios
...
def sample_raw_input_data():
    ...
    return {
        ...
        "director_id": next((p['id'] for p in personnel if p['role'] == 'director'), None),
...
def test_preprocess_input(sample_raw_input_data):
    ...
    df = preprocess_input(sample_raw_input_data, _feature_processor, 
                          _all_genres_list, _all_personnel_list, _all_studios_list)
    assert isinstance(df, pd.DataFrame)
    assert not df.empty
    assert 'budget_usd' in df.columns
    assert 'genre_Action' in df.columns or 'genre_Comedy' in df.columns # Check for some genre columns
...
def test_find_similar_films(sample_raw_input_data):
    ...
    predicted_film_info = {
        'primary_genre_id': sample_raw_input_data['genre_ids'][0] if sample_raw_input_data['genre_ids'] else None,
        'budget_usd': sample_raw_input_data['budget_usd']
    }
    if not predicted_film_info['primary_genre_id']:
        pytest.skip("Skipping test_find_similar_films as no primary genre ID in sample data.")
