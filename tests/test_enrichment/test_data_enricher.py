...
class TestDataEnricher:
    ...
    @patch("scripts.enrich_historical_data.requests.get")
    def test_get_tmdb_details_success(
        self, mock_get, enricher, sample_tmdb_response, sample_credits_response
    ):
    ...
    def test_update_mappings_from_tmdb(
        self, enricher, sample_tmdb_response, sample_credits_response
    ):
        ...
        enricher._update_mappings_from_tmdb(
            sample_tmdb_response, sample_credits_response
        )
    ...
    def test_extract_views_from_kinocheck_dict(self, enricher):
        ...
        result = enricher._extract_views_from_kinocheck({"trailer": {"views": 2000000}})
        assert result == 2000000

        # Test views in videos array
        result = enricher._extract_views_from_kinocheck(
            {"videos": [{"views": 500000}, {"views": 1500000}, {"views": 1000000}]}
        )
    ...
    def test_extract_views_from_kinocheck_list(self, enricher):
        ...
        result = enricher._extract_views_from_kinocheck(
            [{"views": 100000}, {"views": 300000}, {"views": 200000}]
        )
        assert result == 300000  # Should return the max

    def test_enrich_with_tmdb_data(
        self, enricher, sample_tmdb_response, sample_credits_response
    ):
        ...
        enricher.mappings["studios"] = {"Universal Pictures": {"id": 50, "tmdb_id": 33}}
    ...
    def test_enrich_with_tmdb_data_preserves_existing(
        self, enricher, sample_tmdb_response, sample_credits_response
    ):
...
class TestDataEnrichmentIntegration:
    ...
    def test_csv_processing_workflow(self):
        ...
        try:
            ...
            with patch.dict(os.environ, {"TMDB_API_KEY": "test_bearer_token"}):
                ...
                with patch.object(enricher, "get_tmdb_data") as mock_tmdb:
                    ...
                    success = enricher.run_enrichment(
                        input_csv, output_csv, dry_run=False, target_film_id="1"
                    )
