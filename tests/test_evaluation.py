...
def test_model_tracker_evaluate_model(sample_predictions):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        metrics_file = os.path.join(temp_dir, "test_experiment", "metrics", "model_v1_metrics.json")
...
def test_model_tracker_compare_models():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert comparison.loc[comparison["model_version"] == "model_v2", "mae"].iloc[0] < \
...
def test_model_tracker_get_best_model():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for model in ["model_v1", "model_v2", "model_v3"]:
            os.makedirs(os.path.join(temp_dir, "test_experiment", "models", model), exist_ok=True)
        
        # Get best model by MAE (lower is better)
        best_model = tracker.get_best_model_version(metric="mae", higher_is_better=False)
