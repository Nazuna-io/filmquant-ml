...
class FilmQuantMLTrainer:
    ...
    def engineer_features(self):
                    ...
                    if pd.notna(row[col]):
                        ...
                        if actor_name in a_list_actors:
                            ...
                            if col == "actor1_id":  # Lead actor
                                ...
                                score += 1
                features_df.loc[features_df.index == row.name, "star_power_score"] = (
                    score
                )

            print(
                f"  Star power scores range: {features_df['star_power_score'].min()}-{features_df['star_power_score'].max()}"
            )
        ...
        features_df["budget_category_mid"] = (
            (budget_series > 50) & (budget_series <= 150)
        ).astype(int)
        ...
        try:
            ...
            features_df["is_summer_release"] = (
                (dates.dt.month >= 5) & (dates.dt.month <= 8)
            ).astype(int)
            features_df["is_holiday_release"] = (
        ...
        features_df["is_major_studio"] = (
            features_df["studio_id"].isin(major_studios).astype(int)
        )
        ...
        print(
            f"Feature engineering complete. Total features: {len(features_df.columns)}"
        )
    ...
    def prepare_training_data(self, features_df):
        ...
        print(
            f"Films with box office data: {valid_target_mask.sum()}/{len(features_df)}"
        )
        ...
        if "kinocheck_trailer_views" in train_data.columns:
            trailer_views = pd.to_numeric(
                train_data["kinocheck_trailer_views"], errors="coerce"
            )
