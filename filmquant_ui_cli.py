...
def parse_args():
    parser = argparse.ArgumentParser(
        description="FilmQuant ML - Box Office Prediction Interface"
    )
    parser.add_argument(
        "--server-port",
        type=int,
...
def create_feature_vector(
    ...
    try:
        ...
        # Convert trailer views from millions to actual count
        trailer_views_actual = trailer_views * 1_000_000 if trailer_views else 500_000
...
if __name__ == "__main__":
    ...
    logger.info(f"🌐 Server: {args.server_name}:{args.server_port}")
