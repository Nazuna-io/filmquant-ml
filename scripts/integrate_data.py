...
class DataIntegrator:
        ...
        for film_data in collected_data:
            ...
            if "tmdb_data" in film_data:
                ...
                if film_mask.any():
                    ...
                    if (
                        pd.isna(current_rating)
                        or current_rating == "TBD"
                        or current_rating == ""
                    ):
                    ...
                    if new_runtime and (
                        pd.isna(current_runtime)
                        or abs(current_runtime - new_runtime) > 5
                    ):
    ...
    def get_wikipedia_box_office(self, film_title, year=2023):
        ...
        try:
            ...
            alt_search_url = (
                f"https://en.wikipedia.org/wiki/{film_title.replace(' ', '_')}"
            )
            ...
            if "tmdb_data" in film_data:
                ...
                if worldwide_revenue > 0:
                    ...
                    if film_mask.any():
                        ...
                        if pd.isna(
                            df.loc[film_mask, "actual_box_office_domestic_usd"].iloc[0]
                        ):
                            df.loc[film_mask, "actual_box_office_domestic_usd"] = (
                                domestic_estimate
                            )
                            estimates_added += 1
                        ...
                        if pd.isna(
                            df.loc[
                                film_mask, "actual_box_office_international_usd"
                            ].iloc[0]
                        ):
                            df.loc[film_mask, "actual_box_office_international_usd"] = (
                                international_estimate
                            )
                            estimates_added += 1

                        if pd.isna(
                            df.loc[film_mask, "opening_weekend_box_office"].iloc[0]
                        ):
                            df.loc[film_mask, "opening_weekend_box_office"] = (
                                opening_weekend_estimate
                            )
                            estimates_added += 1

                        print(
                            f"  ✅ {title}: Domestic=${domestic_estimate:.1f}M, Intl=${international_estimate:.1f}M"
                        )
