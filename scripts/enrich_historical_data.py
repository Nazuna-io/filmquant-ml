...
class DataEnricher:
    def __init__(self):
        self.tmdb_headers = (
            {"Authorization": f"Bearer {TMDB_API_KEY}"} if TMDB_API_KEY else {}
        )
        ...
        for mapping_type in self.mappings.keys():
            ...
            if os.path.exists(mapping_file):
                try:
                    ...
                    print(
                        f"Loaded {len(self.mappings[mapping_type])} {mapping_type} mappings"
                    )
    ...
    def _search_tmdb_film(self, title: str, year: int = None) -> Optional[int]:
        ...
        try:
            response = requests.get(
                search_url, headers=self.tmdb_headers, params=params
            )
                ...
                for result in results:
                    if result["title"].lower() == title.lower():
                        ...
                        if not year or release_year == str(year):
                            print(
                                f"  ✓ Exact match: {result['title']} ({release_year})"
                            )
                            return result["id"]
    ...
    def _get_tmdb_details(self, tmdb_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            details_response = requests.get(
                details_url, headers=self.tmdb_headers, params={"language": "en-US"}
            )
            ...
            # Combine data
            film_data = {"tmdb_id": tmdb_id, "details": details, "credits": credits}
    ...
    def process_film(self, row: Dict, dry_run: bool = False) -> Dict:
        ...
        if dry_run:
            print(
                "  [DRY RUN] - Would fetch TMDB data, trailer views, and screen counts"
            )
            return enriched_row
    ...
    def _enrich_with_tmdb_data(self, row: Dict, tmdb_data: Dict) -> Dict:
        ...
        for i, genre in enumerate(genres, 1):
            ...
            if not row.get(genre_key):
                row[genre_key] = self.mappings["genres"].get(
        ...
        if directors and not row.get("director_id"):
            ...
            row["director_id"] = (
                self.mappings["people"].get(director_name, {}).get("id", director_name)
            )
        ...
        for i, actor in enumerate(cast[:6], 1):
            ...
            if not row.get(actor_key):
                ...
                row[actor_key] = (
                    self.mappings["people"].get(actor_name, {}).get("id", actor_name)
                )
        ...
        if not row.get("studio_id") and details.get("production_companies"):
            ...
            row["studio_id"] = (
                self.mappings["studios"].get(studio_name, {}).get("id", studio_name)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Enrich film dataset with TMDB and other sources"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--film-id", type=str, help="Process only the film with this ID"
    )
    parser.add_argument(
        "--input", type=str, default=INPUT_CSV, help="Input CSV file path"
    )
    parser.add_argument(
        "--output", type=str, default=OUTPUT_CSV, help="Output CSV file path"
    )
    ...
    if not args.dry_run:
        ...
        if not KINOCHECK_API_KEY:
            print(
                "Warning: KINOCHECK_API_KEY not found - trailer views will be limited"
            )

        if not YOUTUBE_API_KEY:
            print(
                "Warning: YOUTUBE_API_KEY not found - no YouTube fallback for trailer views"
            )

    # Run enrichment
    enricher = DataEnricher()
    success = enricher.run_enrichment(
        args.input, args.output, args.dry_run, args.film_id
    )
