...
class CompleteDataCollector:
    ...
    def get_tmdb_data_for_film(self, film_title, tmdb_id=None):
        ...
        if not tmdb_id:
            ...
            response = self.session.get(search_url, headers=self.tmdb_headers, params=search_params)
        ...
        if releases_response.status_code == 200:
            ...
            us_releases = [r for r in releases_data.get('countries', []) if r.get('iso_3166_1') == 'US']
    ...
    def find_box_office_mojo_url(self, film_title, year=2023):
        ...
        title_clean = film_title.lower().replace(' ', '-').replace(':', '').replace("'", "")
        ...
        for url in potential_urls:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if title_element and film_title.lower() in title_element.get_text().lower():
    ...
    def scrape_box_office_mojo_data(self, url):
        ...
        try:
            ...
            screen_matches = re.findall(r'(\d{1,5})\s*(?:screens?|theaters?)', text_content, re.IGNORECASE)
            if screen_matches:
    ...
    def collect_complete_data_for_film(self, film_row):
        ...
        collected = {
            'title': film_title,
            'original_data': film_row.to_dict()
        }
    ...
    def run_complete_data_collection(self):
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if (idx + 1) % 5 == 0:
                    ...
                    self.save_collected_data(all_collected_data, f"data_collection_checkpoint_{idx+1}.json")
        ...
        # Save final results
        final_path = self.save_collected_data(all_collected_data, "complete_data_collection_results.json")
