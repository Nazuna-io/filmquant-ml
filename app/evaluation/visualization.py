...
def plot_metric_history(
    ...
    if len(model_versions) > 1:
        ...
        p = np.poly1d(z)
        ax.plot(
            range(len(model_versions)),
            p(range(len(model_versions))),
            linestyle="--",
            color="red",
...
def plot_feature_importance_comparison(
    ...
    # Plot heatmap
    sns.heatmap(df_plot, annot=True, cmap="viridis", fmt=".3f", linewidths=0.5, ax=ax)
...
def plot_prediction_interval_coverage(
