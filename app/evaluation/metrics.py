...
def quantile_loss(
    ...
    lower_loss = np.mean(
        np.maximum(
            lower_q * (y_true - y_pred_low), (lower_q - 1) * (y_true - y_pred_low)
        )
    )
    ...
    upper_loss = np.mean(
        np.maximum(
            upper_q * (y_true - y_pred_high), (upper_q - 1) * (y_true - y_pred_high)
        )
    )
...
def evaluate_stratified(
    ...
    for i, stratum in enumerate(unique_strata):
        ...
        if np.sum(mask) > 0:  # Ensure there are samples in this stratum
            stratum_name = (
                strata_names[i]
                if strata_names and i < len(strata_names)
                else str(stratum)
            )
...
def plot_actual_vs_predicted(
    ...
    if y_pred_low is not None and y_pred_high is not None:
        ...
        ax.plot(
            [], [], color="red", alpha=0.5, linewidth=2, label="90% Prediction Interval"
        )
...
def plot_residuals_vs_predicted(
    ...
    try:
        ...
        moving_avg = np.convolve(
            residuals_sorted, np.ones(window_size) / window_size, mode="valid"
        )
