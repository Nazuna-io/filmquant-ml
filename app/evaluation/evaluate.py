...
logger = configure_logging(
    app_name="filmquant_ml.evaluation",
...
def evaluate_model(data: pd.DataFrame, 
                  ...
                  stratify_columns: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    ...
    if "stratified_metrics" in metrics:
        for column, strata_metrics in metrics["stratified_metrics"].items():
            plot_file = os.path.join(plots_dir, f"{model_version}_stratified_{column}.png")
            plot_stratified_metrics(strata_metrics, "mae", f"MAE by {column}", plot_file)
    
    logger.info("Evaluation completed successfully")
    return metrics

def main(data_path: str, model_version: str, true_column: str, pred_column: str,
        pred_low_column: Optional[str] = None, pred_high_column: Optional[str] = None,
        experiment_name: Optional[str] = None, feature_importance_file: Optional[str] = None,
        output_dir: str = "evaluation_results", stratify_columns: Optional[List[str]] = None):
    """
    ...
    try:
        ...
        if experiment_name:
            ...
            tracker.evaluate_model(
                ...
                y_true=data[true_column].values,
                y_pred=data[pred_column].values,
                y_pred_low=data[pred_low_column].values if pred_low_column in data.columns else None,
                y_pred_high=data[pred_high_column].values if pred_high_column in data.columns else None,
            ...
            logger.info(f"Generated evaluation report with model tracker for {model_version}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--true-column", "-t", 
        required=True, 
        help="Name of the column with true values"
    )
    parser.add_argument(
        "--pred-column", "-p", 
        required=True, 
