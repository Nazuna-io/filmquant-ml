...
class ModelTracker:
    ...
    def __init__(self, base_dir: str = None, experiment_name: str = None):
        ...
        self.experiment_name = (
            experiment_name
            or f"filmquant_ml_experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _create_directory_structure(self) -> None:
        ...
        logger.debug(
            f"Created directory structure for experiment: {self.experiment_name}"
        )

    def log_hyperparameters(
        self, model_version: str, hyperparameters: Dict[str, Any]
    ) -> str:
        """
    ...
    def log_model_files(
        self, model_version: str, model_files: Dict[str, str]
    ) -> Dict[str, str]:
        """
    ...
    def evaluate_model(
        ...
        y_pred_high: np.ndarray = None,
        feature_names: List[str] = None,
        feature_importance: Dict[str, float] = None,
        strata: Dict[str, np.ndarray] = None,
        ...
        if generate_plots:
            ...
            plot_actual_vs_predicted(
                y_true, y_pred, y_pred_low, y_pred_high, actual_vs_pred_path
            )
    ...
    def _plot_feature_importance(
        self, feature_importance: Dict[str, float], save_path: str
    ) -> plt.Figure:
        """
        ...
        for i, bar in enumerate(bars):
            width = bar.get_width()
            ax.text(width + width * 0.05, i, f"{width:.3f}", ha="left", va="center")
    ...
    def compare_models(self, model_versions: List[str]) -> pd.DataFrame:
        ...
        for model_version in model_versions:
            metrics_file = os.path.join(
                self.metrics_dir, f"{model_version}_metrics.json"
            )

            if not os.path.exists(metrics_file):
                logger.warning(
                    f"Metrics file not found for model version {model_version}"
                )
                continue
            ...
            hp_file = os.path.join(
                self.models_dir, model_version, "hyperparameters.json"
            )
    ...
    def generate_report(
        self, model_version: str, output_format: str = "json"
    ) -> Dict[str, Any]:
        """
        ...
        if os.path.exists(hp_file):
            ...
            logger.warning(
                f"Hyperparameters file not found for model version {model_version}"
            )
        ...
        if os.path.exists(model_dir):
            ...
            logger.warning(
                f"Model directory not found for model version {model_version}"
            )
        ...
        if os.path.exists(plots_dir):
            ...
            logger.warning(
                f"Plots directory not found for model version {model_version}"
            )
        ...
        if output_format == "html":
            ...
            html_file = os.path.join(
                self.experiment_dir, f"{model_version}_report.html"
            )
    ...
    def _generate_html_report(self, report: Dict[str, Any], model_version: str) -> str:
        ...
        if all(metric in report for metric in key_metrics):
            for metric in key_metrics:
                ...
                value = report[metric]
                formatted_value = (
                    f"${value:,.2f}" if metric in ["mae", "rmse"] else f"{value:.2f}%"
                )
        ...
        if "interval_coverage_percent" in report:
            ...
            html.append(
                "<div class='metric-name'>90% Prediction Interval Coverage</div>"
            )
            html.append(
                f"<div class='metric-value'>{report['interval_coverage_percent']:.2f}%</div>"
            )
            html.append("</div>")
        ...
        if "feature_importance" in report:
            ...
            for feature in report["feature_importance"]:
                html.append(
                    f"<tr><td>{feature['feature']}</td><td>{feature['importance']:.4f}</td></tr>"
                )
    ...
    def get_best_model_version(
        self, metric: str = "mae", higher_is_better: bool = False
    ) -> Optional[str]:
        """
        ...
        for model_version in model_versions:
            ...
            if metric not in metrics:
                logger.warning(
                    f"Metric '{metric}' not found for model version {model_version}"
                )
                continue
