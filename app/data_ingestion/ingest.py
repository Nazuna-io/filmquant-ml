...
logger = configure_logging(
    app_name="filmquant_ml.data_ingestion", log_level=logging.INFO
)
...
def create_pipeline(config: Dict[str, Any]) -> DataIngestionPipeline:
    ...
    if config.get("cleaning", {}).get("enabled", True):
        ...
        pipeline.add_processor(
            CleaningProcessor(
                drop_na_columns=cleaning_config.get(
    ...
    if config.get("categorical_encoding", {}).get("enabled", True):
        ...
        pipeline.add_processor(
            CategoricalEncodingProcessor(
                categorical_columns=cat_config.get(
                    "categorical_columns", ["genre_ids", "studio_id"]
                ),
    ...
    if config.get("numerical_features", {}).get("enabled", True):
        ...
        pipeline.add_processor(
            NumericalFeatureProcessor(
                numerical_columns=num_config.get(
                    "numerical_columns", ["budget_usd", "runtime_minutes"]
                ),
                transformations=num_config.get(
                    "transformations", {"log": ["budget_usd", "box_office_revenue"]}
                ),
            )
        )
...
def load_data(file_path: str) -> pd.DataFrame:
    ...
    try:
        if file_ext == ".csv":
            ...
            raise ValueError(
                f"Unsupported file format: {file_ext}, expected .csv or .json"
            )
...
def main(input_path: str, output_path: str, config_path: str = None):
    ...
    logger.info(
        f"Config: {config_path if config_path else 'Using default configuration'}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", "-i", required=True, help="Path to the input data file (CSV or JSON)"
    )
    parser.add_argument(
        "--output", "-o", required=True, help="Path to save the processed data (CSV)"
    )
    parser.add_argument(
        "--config",
        "-c",
