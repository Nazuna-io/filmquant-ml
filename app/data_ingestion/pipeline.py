...
class DataIngestionPipeline:
    ...
    """
    def __init__(self, processors: List[DataProcessor] = None, name: str = "DataIngestionPipeline"):
        """
    ...
    def run(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            Processed DataFrame
        """
        self.logger.info(f"Starting pipeline run with {len(self.processors)} processors")
        result = data.copy()
        
        for i, processor in enumerate(self.processors):
            self.logger.info(f"Running processor {i+1}/{len(self.processors)}: {processor.name}")
            try:
                result = processor(result)
                self.logger.info(f"Processor {processor.name} completed successfully")
            except Exception as e:
                self.logger.error(f"Error in processor {processor.name}: {str(e)}", exc_info=True)
                raise
    ...
    def save_processed_data(self, data: pd.DataFrame, output_path: str) -> None:
        ...
        try:
            if ext == '.csv':
                ...
                self.logger.warning(f"Unrecognized file extension: {ext}, defaulting to CSV")
            ...
            self.logger.error(f"Error saving data to {safe_path}: {str(e)}", exc_info=True)
            raise
