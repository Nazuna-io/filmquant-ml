...
class CleaningProcessor(DataProcessor):
    ...
    """
    def __init__(self, drop_na_columns: List[str] = None, fill_na_values: Dict[str, Any] = None):
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.drop_na_columns:
            ...
            self.logger.info(f"Dropped {dropped_rows} rows with NaN values in columns: {self.drop_na_columns}")
        ...
        for column, fill_value in self.fill_na_values.items():
            if column in df.columns:
                ...
                if missing_count > 0:
                    ...
                    self.logger.info(f"Filled {missing_count} NaN values in '{column}' with '{fill_value}'")
        ...
        for date_col in self.date_columns:
            ...
            if not pd.api.types.is_datetime64_any_dtype(df[date_col]):
                try:
                    ...
                    self.logger.error(f"Failed to convert '{date_col}' to datetime: {str(e)}")
                    continue
            ...
            if self.output_format == 'onehot':
                # Month one-hot
                month_dummies = pd.get_dummies(df[f"{col_prefix}month"], prefix=f"{col_prefix}month")
                df = pd.concat([df, month_dummies], axis=1)
                
                # Quarter one-hot
                quarter_dummies = pd.get_dummies(df[f"{col_prefix}quarter"], prefix=f"{col_prefix}quarter")
                df = pd.concat([df, quarter_dummies], axis=1)
                
                # Season one-hot
                season_dummies = pd.get_dummies(df[f"{col_prefix}season"], prefix=f"{col_prefix}season")
...
class CategoricalEncodingProcessor(DataProcessor):
    ...
    - Target encoding
    """
    def __init__(self, categorical_columns: List[str], method: str = "onehot", target_column: str = None):
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.categorical_columns:
            ...
            if self.method == 'onehot':
                ...
                self.logger.debug(f"One-hot encoded '{col}' into {dummies.shape[1]} columns")
                ...
                self.logger.debug(f"Label encoded '{col}' with {len(category_map)} categories")
                ...
                self.logger.debug(f"Target encoded '{col}' using '{self.target_column}'")
...
class NumericalFeatureProcessor(DataProcessor):
    ...
    - Polynomial features
    """
    def __init__(self, numerical_columns: List[str], transformations: Dict[str, List[str]]):
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'log' in self.transformations:
            for col in self.transformations['log']:
                ...
                if (df[col] <= 0).any():
                    ...
                    self.logger.debug(f"Applied log1p transformation to '{col}' -> '{log_col}'")
                else:
                    log_col = f"{col}_log"
                    df[log_col] = np.log(df[col])
                    self.logger.debug(f"Applied log transformation to '{col}' -> '{log_col}'")
        ...
        if 'minmax' in self.transformations:
            for col in self.transformations['minmax']:
                ...
                self.logger.debug(f"Applied min-max scaling to '{col}' -> '{scaled_col}'")
        ...
        if 'bins' in self.transformations:
            for col in self.transformations['bins']:
                ...
                if n_bins < 2:
                    self.logger.warning(f"Not enough unique values in '{col}' for binning")
                    continue
                
                # Create bins
                binned_col = f"{col}_binned"
                df[binned_col] = pd.qcut(df[col], q=n_bins, duplicates='drop', labels=False)
                self.logger.debug(f"Applied binning to '{col}' -> '{binned_col}' with {n_bins} bins")
        
        # Apply standard scaling
        if 'standard' in self.transformations:
            for col in self.transformations['standard']:
                if col not in df.columns:
                    self.logger.warning(f"Column '{col}' not found for standard scaling")
                    continue
                
                col_mean = df[col].mean()
                col_std = df[col].std()
                if col_std == 0:
                    self.logger.warning(f"Standard deviation is 0 for '{col}', skipping scaling")
                    continue
                ...
                self.logger.debug(f"Applied standard scaling to '{col}' -> '{scaled_col}'")
...
class DataSplitProcessor(DataProcessor):
    ...
    def __init__(self, target_column: str, test_size: float = 0.2, random_state: int = 42, 
                 split_method: str = "random", split_column: str = None):
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.target_column not in df.columns:
            self.logger.error(f"Target column '{self.target_column}' not found in DataFrame")
        ...
        if self.split_method == 'random':
            ...
            self.logger.info(f"Randomly split data: {(~mask).sum()} train, {mask.sum()} test samples")
            ...
            if self.split_column not in df.columns:
                self.logger.error(f"Split column '{self.split_column}' not found for time-based splitting")
            ...
            self.logger.info(f"Time-split data: {len(df) - test_count} train, {test_count} test samples")
            
        elif self.split_method == 'stratified':
            # Stratified split (ensures same distribution of split_column in train/test)
            if self.split_column not in df.columns:
                self.logger.error(f"Split column '{self.split_column}' not found for stratified splitting")
                raise ValueError(f"Split column '{self.split_column}' not found")
            ...
            for stratum in strata:
                ...
                np.random.seed(self.random_state)
                test_indices = np.random.choice(stratum_indices, test_count, replace=False)
                df.loc[test_indices, 'split'] = 'test'
            
            test_count = (df['split'] == 'test').sum()
            self.logger.info(f"Stratified-split data: {len(df) - test_count} train, {test_count} test samples")
...
class FeatureSelectionProcessor(DataProcessor):
    ...
    """
    def __init__(self, target_column: str, method: str = "correlation", threshold: float = 0.05, 
                 max_features: int = None, include_columns: List[str] = None):
        """
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        numerical_cols = [col for col in numerical_cols if col != self.target_column and col != 'split']
        
        if self.method == 'correlation':
            # Calculate correlation with target
            correlations = df[numerical_cols + [self.target_column]].corr()[self.target_column].abs()
            correlations = correlations.drop(self.target_column)
            
            # Select features based on correlation threshold
            selected = correlations[correlations > self.threshold].index.tolist()
            self.logger.info(f"Selected {len(selected)} features based on correlation > {self.threshold}")
            
            # If max_features is set, take top N features
            if self.max_features and len(selected) > self.max_features:
                selected = correlations.sort_values(ascending=False).head(self.max_features).index.tolist()
                self.logger.info(f"Reduced to top {self.max_features} features by correlation")
            ...
            self.logger.info(f"Selected {len(selected)} features based on variance > {self.threshold}")
            
            # If max_features is set, take top N features
            if self.max_features and len(selected) > self.max_features:
                selected = variances.sort_values(ascending=False).head(self.max_features).index.tolist()
                self.logger.info(f"Reduced to top {self.max_features} features by variance")
            ...
            self.logger.info(f"Using all {len(selected)} numerical features (no selection applied)")
