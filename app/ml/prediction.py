...
class MockXGBoostBooster:
    ...
    def __init__(self, model_path):
        ...
        self.feature_names = [
            'budget_usd', 'runtime_minutes', 'screens_opening_day', 'marketing_budget_est_usd',
            'marketing_budget_is_estimated', 'trailer_views_prerelease', 'genre_Action', 'genre_Comedy', 
            'genre_Drama', 'genre_Sci-Fi', 'genre_Thriller', 'director_id_encoded', 'actor1_id_encoded',
            'actor2_id_encoded', 'actor3_id_encoded', 'studio_id_encoded', 'release_month',
            'release_quarter_1', 'release_quarter_2', 'release_quarter_3', 'release_quarter_4',
            'avg_cast_notable_works', 'director_notable_works'
        ]
    ...
    def predict(self, dmatrix):
        ...
        if 'marketing_budget_est_usd' in dmatrix:
            ...
            marketing_is_estimated = dmatrix.get('marketing_budget_is_estimated', pd.Series([0])).iloc[0] == 1
...
class MockFeatureProcessor:
    ...
    def transform(self, X_df):
        ...
        for col in ['budget_usd', 'runtime_minutes', 'screens_opening_day', 
...
def load_feature_processor_from_gcs(bucket_name=None, processor_path=None):
    ...
    if not os.path.exists(PROCESSOR_FILE):
        raise FileNotFoundError(f"Feature processor placeholder not found: {PROCESSOR_FILE}")
...
def preprocess_input(raw_input_data: dict, feature_processor: MockFeatureProcessor, 
                       all_genres_list: list, all_personnel_list: list, all_studios_list: list) -> pd.DataFrame:
    """
    ...
    if marketing_budget is None or marketing_budget == 0:
        ...
        processed_data['marketing_budget_is_estimated'] = 1  # Flag to indicate this is an estimate
    else:
    ...
    processed_data['trailer_views_prerelease'] = raw_input_data.get('trailer_views_prerelease', 0) # Handle missing
    ...
    for genre_obj in all_genres_list:
        genre_name_slug = genre_obj['name'].replace(' ', '_').replace('-', '_') # Simple slug for col name
        processed_data[f'genre_{genre_name_slug}'] = 1 if genre_obj['id'] in input_genre_ids else 0
    ...
    processed_data['director_id_encoded'] = personnel_id_to_idx.get(raw_input_data.get('director_id'), -1) # -1 for unknown
    ...
    processed_data['studio_id_encoded'] = studio_id_to_idx.get(raw_input_data.get('studio_id'), -1)
    ...
    if release_date_str:
        try:
            ...
            for q_val in range(1, 5):
                 processed_data[f'release_quarter_{q_val}'] = 1 if quarter == q_val else 0
        except ValueError:
            processed_data['release_month'] = -1 # Or some other indicator for invalid date
            for q_val in range(1, 5):
    ...
    processed_data['director_notable_works'] = director_obj.get('notable_works_count', 0) if director_obj else 0
    ...
    for actor_id in cast_ids[:3]: # Max 3 actors
        ...
        if actor_obj:
            ...
            valid_cast_count += 1
    processed_data['avg_cast_notable_works'] = avg_cast_notable_works / valid_cast_count if valid_cast_count > 0 else 0
    ...
    expected_feature_order = _model.feature_names if _model else feature_processor.expected_features_
    ...
    feature_processor.expected_features_ = expected_feature_order # Store for consistency if needed
    return final_df
...
def extract_quantile_predictions(predictions_array):
    ...
    """
    if predictions_array is None or len(predictions_array) == 0 or len(predictions_array[0]) < 3:
    ...
    return predicted_revenue_usd, confidence_interval_low_usd, confidence_interval_high_usd

def get_top_factors_xgboost(model: MockXGBoostBooster, feature_names: list, 
                              user_input_values_dict: dict, top_n=5, use_shap=False):
    """
    ...
    for feature_encoded_name, importance_score in sorted_factors[:top_n]:
        ...
        if feature_encoded_name.startswith('genre_'):
            display_name = f"Genre: {feature_encoded_name.split('genre_')[1].replace('_', ' ')}"
            ...
            for g in _all_genres_list:
                if g['name'].replace(' ', '_').replace('-', '_') == genre_name_part:
                    ...
                    break
            if genre_id_for_name and genre_id_for_name in user_input_values_dict.get('genre_ids', []):
                value_contributed = 1
            else:
                value_contributed = 0
        elif feature_encoded_name == 'director_id_encoded':
            director_id = user_input_values_dict.get('director_id')
            director = next((p['name'] for p in _all_personnel_list if p['id'] == director_id), 'Unknown Director')
...
def find_similar_films(predicted_film_data: dict, historical_films_list: list, top_n=3) -> list:
    """
    ...
    primary_genre_id = predicted_film_data.get('primary_genre_id') # Assumes this is determined and passed in
    ...
    for film in candidates[:top_n]:
        similar_films_output.append({
            ...
            "predicted_or_actual_revenue_usd": film.get('actual_box_office_total_usd'),
...
def get_prediction(raw_input_data: dict):
    ...
    processed_df = preprocess_input(raw_input_data, _feature_processor, 
                                    _all_genres_list, _all_personnel_list, _all_studios_list)
    ...
    predicted_revenue, conf_low, conf_high = extract_quantile_predictions(model_predictions)
    ...
    top_factors = get_top_factors_xgboost(_model, list(processed_df.columns), raw_input_data, top_n=5)
    ...
    comparable_films = find_similar_films(predicted_film_info_for_similarity, historical_film_data, top_n=3)
...
if __name__ == '__main__':
    ...
    sample_raw_input = {
        ...
        "director_id": _all_personnel_list[0]['id'] if _all_personnel_list else "p_unknown",
