...
async def verify_api_key(
    ...
    if not path.startswith(api_prefix) or any(
        path.startswith(route) for route in unprotected_routes
    ):
        return
    ...
    if api_key != valid_api_key:
        logger.warning(
            f"Unauthorized access attempt to {path} - Invalid API key provided"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
