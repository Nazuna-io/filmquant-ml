...
class PredictRequest(BaseModel):
    ...
    genre_ids: List[str] = Field(
        ...,
        min_items=1,
        description="List of genre IDs. At least one genre is required.",
    )
    ...
    cast_ids: List[str] = Field(
        ...
        min_items=1,
        max_items=10,
        description="List of cast member IDs. At least one is required.",
    )
    ...
    marketing_budget_est_usd: Optional[confloat(ge=0)] = Field(
        None,
        description="Estimated marketing budget in USD. Must be non-negative if provided.",
    )
    trailer_views_prerelease: Optional[conint(ge=0)] = Field(
        None,
        description="Number of trailer views before release. Must be non-negative if provided.",
    )
    ...
    def validate_director_id(cls, v):
        ...
        valid_directors = {
            p["id"] for p in get_cast_and_crew() if p["role"] == "director"
        }
...
class ValidateRequest(BaseModel):
    ...
    override_features: Optional[Dict[str, Any]] = Field(
        ...
        description="Features to override in the historical film data",
    )
    ...
    def validate_historical_film_id(cls, v):
        ...
        if not any(film.get("id") == v for film in historical_films):
            raise ValueError(
                f"Historical film with ID '{v}' not found in reference data"
            )
        return v
...
@router.post(
    "/predict", status_code=status.HTTP_200_OK, response_model_exclude_none=True
)
async def predict_route(payload: PredictRequest):
    ...
    try:
        ...
        logger.info(
            f"Successfully generated prediction for {payload.title} with revenue: {prediction_output.get('predicted_revenue_usd')}"
        )
...
async def validate_route(payload: ValidateRequest):
    ...
    try:
        logger.info(
            f"Validating prediction for historical film ID: {payload.historical_film_id}"
        )
        ...
        historical_films = get_historical_films()
        target_film = next(
            (film for film in historical_films if film.get("id") == historical_film_id),
            None,
        )
        ...
        fields_to_remove = [
            ...
            "actual_box_office_domestic_usd",
            "actual_box_office_international_usd",
            "id",
            "budget_difference",
        ]
        ...
        if "title" not in film_data_for_prediction:
            film_data_for_prediction["title"] = target_film.get(
                "title", "Unknown Historical Title"
            )
        ...
        if actual_revenue and predicted_revenue:
            ...
            conf_high = prediction_output.get(
                "confidence_interval_high_usd", float("inf")
            )
        ...
        validation_response = {
            ...
            "confidence_interval_low_usd": prediction_output.get(
                "confidence_interval_low_usd"
            ),
            "confidence_interval_high_usd": prediction_output.get(
        ...
        logger.info(
            f"Validation completed for film '{target_film.get('title')}' with APE: {absolute_percentage_error}%"
        )
