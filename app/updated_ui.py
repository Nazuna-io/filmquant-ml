...
def create_updated_filmquant_ml_interface(callbacks):
    ...
    historical_film_choices = [
        (f"{film['title']} ({film.get('release_date', 'Unknown')[:4]})", film["id"])
    ...
    with gr.Blocks(
            ...
            with gr.TabItem("🎯 Predict New Film", id=0):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        ...
                        director_dropdown, director_buttons = (
                            create_quick_select_component(
                                "Director",
                                a_list_directors,
                                all_directors,
                                "film-director",
                    ...
                    with gr.Column(scale=1):
                        ...
                        with gr.Accordion("🔍 Technical Details", open=False):
                            raw_input_display_output = gr.JSON(
                                label="Model Input Data", elem_id="raw-input"
                            )
        ...
        def historical_with_million_conversion(film_id):
            ...
            if result and len(result) >= 8:
                ...
                if (
                    len(result_list) > 11 and result_list[11] is not None
                ):  # marketing budget
