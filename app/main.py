...
from app.ml.prediction import (
    get_prediction,  # We can call this directly from Gradio callbacks
)
...
def predict_button_callback(
    ...
    raw_input_data = {
        ...
        "genre_ids": (
            ...
            else [genre_ids] if genre_ids else []
        ),
        ...
        "trailer_views_prerelease": (
            trailer_views_prerelease if trailer_views_prerelease else 0
        ),
    }
    ...
    try:
        ...
        logger.info(
            f"Prediction successful for '{title}' with revenue: ${prediction_output.get('predicted_revenue_usd', 0):,.0f}"
        )
    ...
    domestic_share = (
        f"{prediction_output.get('domestic_revenue_share_percent', 0)*100:.1f}%"
    )
    international_share = (
        f"{prediction_output.get('international_revenue_share_percent', 0)*100:.1f}%"
    )
    ...
    for film in comparable_films_data:
        if "predicted_or_actual_revenue_usd" in film and isinstance(
            film["predicted_or_actual_revenue_usd"], (int, float)
        ):
            film["predicted_or_actual_revenue_usd"] = (
                f"${film['predicted_or_actual_revenue_usd']:,.0f}"
            )
    comparable_films_df = pd.DataFrame(comparable_films_data)

    raw_input_display = json.dumps(
        prediction_output.get("raw_input_data", {}), indent=2
    )
...
def historical_film_selected_callback(historical_film_id):
    ...
    film_detail = next(
        (f for f in get_historical_films() if f["id"] == historical_film_id), None
    )
    if not film_detail:
        logger.warning(
            f"Historical film with ID {historical_film_id} not found in reference data"
        )
        return (
            "",
            [],
            ...
            0,
            0,
            f"Film ID {historical_film_id} not found.",
        )
    ...
    logger.info(
        f"Historical film '{film_detail.get('title')}' data loaded successfully"
    )
    return (
        ...
        (
            datetime.strptime(film_detail["release_date"], "%Y-%m-%d")
            if film_detail.get("release_date")
            else None
        ),
